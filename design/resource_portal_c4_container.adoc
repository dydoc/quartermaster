...
* `spec.previousResourceVersion` — `resourceVersion` of the `ProvisionedResource` at snapshot time; used by the controller to detect stale rollback targets (update only)
...
The `ProvisionedResource` carries a finalizer (`portal.example.com/deprovisioning`) added by the controller at creation time. The finalizer is removed only after the controller has confirmed that the CRD target instance on the downstream cluster has been successfully deleted. The Provisioning Handler checks for `NotFound` before entering the watch loop — if the instance is already absent (e.g. after a crash-restart), the handler treats this as a confirmed deletion and proceeds directly to finalizer removal, making the delete path fully idempotent.
...
3. Web App snapshots `appliedParameters` and `resourceVersion` into `previousParameters` and `previousResourceVersion`, creates a `ResourceRequest` (`action: update`, `targetRef` with UID).
4. **Approver** sees a diff view (new parameters vs `previousParameters`), approves it.
5. Controller validates `previousResourceVersion` against the current `ProvisionedResource.resourceVersion`. If they differ, another update completed between submission and approval — the controller sets phase to `failed` with `failureReason = stale rollback target` rather than proceeding with a potentially incorrect rollback target.
6. Controller advances to `provisioning`, applies the updated CRD target instance via SSA.
7. Controller watches for `Ready` confirmation. On success, updates `ProvisionedResource` with new `appliedParameters`, sends notification.
...
5. If present, controller deletes the CRD target instance via Rancher and watches for it to disappear.
...
**`previousResourceVersion` guards against stale rollback targets.** The Web App snapshots the `ProvisionedResource.resourceVersion` at submission time. If another update completes and is confirmed between submission and approval, the controller detects the mismatch and fails the request rather than proceeding with a rollback target that no longer reflects the actual pre-update state.

**Deprovisioning is idempotent on `NotFound`.** The Provisioning Handler checks for `NotFound` before entering the delete watch loop. This makes the delete path safe to re-run after a crash-restart without special-casing the controller logic.
