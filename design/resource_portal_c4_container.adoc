...
* `spec.requester` — identity of the user who submitted the request, extracted from the OIDC token at submission time; used by the Notifier to address state-change emails
* `status.phase` — `pending` → `approved` / `rejected` → `provisioning` → `provisioned` / `rolling_back` → `failed`
+
...
+
NOTE: `rolling_back` is an `update`-only intermediate phase, set when provisioning retries are exhausted and the controller is re-applying `previousParameters` to restore the CRD target instance to its pre-update state. It is a real `status.phase` value, visible to the End User in the portal. `ProvisionedResource` retains `appliedParameters` throughout — it is never updated optimistically. Once the rollback apply is confirmed, the controller advances to `failed`.
...
The `ProvisionedResource` carries a finalizer (`portal.example.com/deprovisioning`) added by the controller at creation time. The finalizer is removed only after the controller has confirmed that the CRD target instance on the downstream cluster has been successfully deleted.
