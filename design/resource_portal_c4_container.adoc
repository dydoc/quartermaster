...
The Web Application is a monolith that serves all HTTP traffic. It is server-side rendered using the standard `html/template` package — there is no separate frontend bundle or SPA framework. A single process handles session management, OIDC authentication, catalogue browsing, request submission, approval actions, and status display.
...
**CEL immutability.** The following `spec` fields are marked immutable after creation via `x-kubernetes-validations` rules: `action`, `targetRef`, `resourceType`, `approvers`, `approvalPolicy`, `requester`. This prevents any actor from retroactively modifying the governance perimeter of an in-flight request.
