...
**Governance layer for CRD and operator installation.** The current scope of the portal is the governance of CR _instance_ creation — consuming pre-installed infrastructure APIs. A natural extension of the same model is governing the installation of CRDs and operators themselves: "we want to install the `DatabaseClaim` CRD and its controller on cluster X" becomes a portal request subject to the same approval workflow, quota model, and audit trail.

This is architecturally coherent with the current design. A CRD + operator installation is just another `ResourceCatalogue` entry whose parameter schema captures chart values and whose CRD target instance is a Fleet `GitRepo`/Fleet Bundle or equivalent GitOps object. The Approval Controller would apply it through the same provisioning path it uses for any other CR — delegating execution to Fleet rather than to a bespoke operator.
