...
image::seq_submission.svg[title="Sequence diagram for the request submission flow in the Web Application. The diagram shows the interaction between the End User, the HTTP Handler, the Submission Handler, the Approver Resolver, and the Kubernetes API server (via Rancher). Key steps highlighted: parameter validation against the ResourceCatalogue schema, the single-active-request constraint check on the informer cache, the previousParameters and previousResourceVersion snapshot for update actions, approver list resolution, and the final ResourceRequest write authenticated as the logged-in user.",align="center"]
...
**previousParameters and previousResourceVersion snapshotted by the Submission Handler.** For `update` actions, the Submission Handler reads the current `ProvisionedResource.spec.appliedParameters` and `metadata.resourceVersion` and writes them into `ResourceRequest.spec.previousParameters` and `spec.previousResourceVersion` at submission time. The parameters are the source of truth for the approver diff view and for the controller's rollback logic. The `resourceVersion` allows the controller to detect whether another update completed between submission and approval, making the rollback target stale before any provisioning work begins.
...
image::reconcile_update.svg[title="Sequence diagram for the Approval Controller reconcile loop handling an update action, including the failure and rollback paths. The diagram shows the Concurrency Guard check at the entry point, the forward SSA apply of new parameters, the confirmation watch loop, and — on exhausted retries — the phase transition to rolling_back followed by the Rollback Handler re-applying previousParameters. If rollback also exhausts retries, the phase transitions to rollback_failed. The ProvisionedResource is never updated optimistically.",align="center"]

image::reconcile_delete.svg[title="Sequence diagram for the Approval Controller reconcile loop handling a delete action. The diagram shows the Concurrency Guard check, the NotFound idempotency check before entering the deletion watch loop, finalizer removal after confirmed absence, and the failure path when deletion retries are exhausted.",align="center"]
...
NOTE: The Reconciler does not use an internal watch loop to wait for CRD target instance readiness. Instead it exits with `ctrl.Result{RequeueAfter: interval}` and re-evaluates state on the next invocation. This avoids goroutine leaks that would result from long-running watches inside the reconcile function — `controller-runtime` may cancel the reconcile context at any time (e.g. on graceful shutdown), and a goroutine blocked on a watch would not be cleaned up.
...
For `update` actions transitioning from `approved` to `provisioning`, the state machine validates `spec.previousResourceVersion` against the current `ProvisionedResource.resourceVersion` before proceeding. If they differ, the state machine transitions directly to `failed` with `failureReason = stale rollback target` — the Provisioning Handler is never invoked.

**Provisioning Handler** executes the actual work when the state machine determines that provisioning, updating, or deleting a target-cluster resource is required. It constructs the CRD target manifest from `ResourceRequest.spec.parameters` and applies it to the target downstream cluster via SSA, using the controller's own service account through Rancher. It exits with `ctrl.Result{RequeueAfter: interval}` rather than blocking on a watch loop — readiness is evaluated on the next reconcile invocation by reading the CRD target instance `status.conditions`. On confirmed success it signals the ProvisionedResource Writer. On exhausted retries for an `update` action it sets the `ResourceRequest` phase to `rolling_back` and invokes the Rollback Handler.
...
**No blocking watch loops in the Reconciler.** All readiness checks are done by exiting with `ctrl.Result{RequeueAfter: interval}` and re-evaluating on the next invocation. This makes the Reconciler safe to cancel at any point, avoids goroutine leaks, and makes the reconcile function straightforward to test without a running cluster.
