...
image::reconcile_update.svg[title="Sequence diagram for the Approval Controller reconcile loop handling an update action, including the failure and rollback path. The diagram shows the Concurrency Guard check at the entry point, the forward SSA apply of new parameters, the confirmation watch loop, and — on exhausted retries — the phase transition to rolling_back followed by the Rollback Handler re-applying previousParameters to restore the CRD target instance. The ProvisionedResource is never updated optimistically: it retains the last confirmed appliedParameters throughout. The phase advances to failed only after rollback is confirmed Ready on the target cluster.",align="center"]
